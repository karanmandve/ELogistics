...
export class ProfileComponent {
  ...
  sanitizeField(fieldName: string): void {
    this.updateCustomerForm.get(fieldName)?.valueChanges.subscribe((value) => {
      if (value) {
        ...
        if (value !== sanitizedValue) {
          this.updateCustomerForm.get(fieldName)?.setValue(sanitizedValue, {
            emitEvent: false, 
          });
        }
      }
  ...
  onFileSelected(event: Event) {
    ...
    if (file) {
      ...
      if (!validTypes.includes(file.type)) {
        this.updateCustomerForm.get('imageFile')?.setErrors({ invalidType: true });
        if (event.target instanceof HTMLInputElement) {
          event.target.value = ''; // Clear invalid input
        }
        // return; // Exit if invalid type
      } else {
        this.updateCustomerForm.get('imageFile')?.setErrors(null);
      }
      ...
      this.updateCustomerForm.patchValue({ imageFile: file });
      this.updateCustomerForm.get('imageFile')?.updateValueAndValidity();
  ...
  openCustomerUpdateModal(): void {
    const modalElement = document.getElementById('customerUpdateModal');
  ...
  onEditProfile(){
    this.openCustomerUpdateModal();
    this.updateCustomerForm.patchValue(this.userDetails)
  ...
  onUpdateSubmit(){
    ...
    Object.keys(this.updateCustomerForm.controls).forEach((field) => {
      if (field === "id"){
        ...
        const value = this.updateCustomerForm.get(field)?.value;
        formData.append(field, value);
      }
    });
